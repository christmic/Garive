...
use crate::{
    ...
    plan_fail_owned_step_from_turn, plan_succeed_goal_from_completed_plan,
    reconstruct_execution_work_binding, reconstruct_goal, reconstruct_local_start,
    reconstruct_plan_graph, AuthorityPort, CommittedTurn, ExecutorPort, F0ExecutionGovernance,
    F0GovernanceContext, F0RecoveryContentPort, GetTurnQuery, GoalCommandContext,
...
impl LocalExecutionWorker {
    ...
    fn reduce_completed_plan_step(
        ...
        positions.extend(self.drive_plan_goal_terminals(
            ledger,
            committed,
            binding.goal_id(),
            recorded_at,
    ...
    fn drive_plan_goal_terminals(
        ...
        let mut goal = reconstruct_goal(ledger, &committed.session_id, goal_id)
            .map_err(|_| LocalWorkerError::PlanCoordinationFailed)?;
        if goal.snapshot.state() == GoalState::Succeeded {
            return Ok(positions);
        }
