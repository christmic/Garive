...
func TestAPNSSenderUsesBackgroundHeadersAndContentFreePayload(t *testing.T) {
	...
	hint := testWakeHint()
	registration := PushRegistration{Transport: "apns", Token: strings.Repeat("ab", 32)}
	if err = sender.Send(context.Background(), registration, hint); err != nil {
		t.Fatal(err)
	}
	if observed.URL.Host != "api.push.apple.com" ||
		observed.Header.Get("apns-push-type") != "background" || observed.Header.Get("apns-priority") != "5" ||
	...
	if json.Unmarshal(payload, &body) != nil || strings.Contains(string(payload), "session") ||
		!strings.Contains(string(payload), `"content-available":1`) || !strings.Contains(string(payload), hint.RouteToken) {
...
func TestFCMSenderUsesOAuthDataMessageAndCachesToken(t *testing.T) {
	...
	registration := PushRegistration{Transport: "fcm", Token: "long-enough-fcm-registration-token"}
	...
	if strings.Contains(contents, "notification") || strings.Contains(contents, "session") ||
		!strings.Contains(contents, `"priority":"normal"`) || !strings.Contains(contents, `"route_token"`) {
...
func TestProviderFailuresExposeNoRemoteResponse(t *testing.T) {
	...
	err = sender.Send(context.Background(), PushRegistration{Transport: "fcm", Token: "long-enough-fcm-registration-token"}, testWakeHint())
