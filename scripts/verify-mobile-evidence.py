...
def verify(artifacts: bool) -> None:
    ...
    if len(files) != 36 or "当前手册包含 36 张实际运行截图" not in text:
        raise ValueError(f"manual must contain and declare exactly 36 screenshots, found {len(files)}")
    ...
    if "complete 36-screenshot user guide" not in status:
