...
|---|---|---|
| C7 measured compression | gated | C7-A supplies the corpus/runner, P2-VX-ATC the exact vendor exchange, C7-B the secret-safe normal-provider composition, and delivered C7-C the OS credential reference, bounded HTTPS and clean-revision attestation. Remaining: execute and review a credentialed publication run, then specify measured trigger/retention trade-offs. |
| Creativity | gated | CR-A delivers the neutral four-class paired harness; CR-B delivers normal compatible-Provider model ports, OS credential references, clean-revision attestation and publication evidence v2. Remaining: install reviewed generator/evaluator coordinates and credentials, execute representative paired runs, then review correctness/diversity trade-offs and model-revision evidence before any production behavior Spec. Harness/loopback tests are not that baseline. |
