...
| F0 | Sandbox enforcement and safety governance | accepted | accepted | documented | partial | partial | active | Rust/Kotlin profiles and Prepared-v3 bindings plus Runtime preflight are green. Prepared-v3, Safety, Grant-v2, Binding and Preflight payloads fail closed in both Ledgers; Rust enforces their exact transition order and plans the full durable admission batch. Mandatory governed-effect-port consumption, crash recovery projection and concrete native sandbox executors remain. |
| G1 | Durable Goal lifecycle | accepted | accepted | documented | partial | partial | active | Shared Rust/Kotlin values, reducer and L0 facts plus SQLite command/restart/race evidence are green; parent narrowing, corruption injection and client projection remain before done. |
| PL1 | Durable executable Plan | accepted | accepted | documented | partial | partial | active | Rust/Kotlin canonical DAG/reducer and 15 strict L0 facts are green. SQLite proposal/adoption/claim/terminal paths, monotonic expiry, restart, competing claims, exact replay and bounded retry pass. Step Start atomically creates its bound C6 Turn/Execution and recovery verifies one SQLite commit version; the unbound Start path is closed. The F0 pre-start fact chain is executable; mandatory effect-port consumption, carry-forward and client projection remain. |
...
| A-MOBILE-R | Native remote-work mobile product | accepted | accepted | documented | implemented | partial | active | KMP controller, secure pairing, native Compose/SwiftUI product, installable builds, Gateway/private APNs/FCM wake paths, complete 37-screenshot user guide, real native loopback actions, offline/reconnect, desktop-aligned drawer and goal starters, local Session search/status filters, explicit rendered-conversation sharing, dark/200%-equivalent type, Android landscape plus persistent-sidebar Android tablet/iPad evidence are verified locally. Ordinary Session rows and accessibility labels hide internal IDs. Agent details are exact and copyable, task creation preserves the selected definition, and remote submission uses one consistent 16 KiB UI/controller input boundary within a 64 KiB command envelope. Explicit approve/decline, collapsed Activity, multi-Turn append, cancellation and stable dismissible failure banners are directly exercised. Ambiguous mutations persist strict digest-bound identity/input records across process restart, including two-stage create/start recovery, exact retry and warned local abandonment; banner dismissal never clears pending work. Authentication failure returns both native shells to pairing. Pairing links on both platforms now pass one shared canonical HTTPS-origin boundary before presentation; Android singleTop delivery and visible service confirmation are exercised directly. Settings includes persistent theme, system notification controls, verified host, safe diagnostics and confirmed unpair; Android Keystore and Apple Keychain tests prove credential deletion and device-key rotation. Current local gates include 16 standard API 36 native tests plus four opt-in real-HTTP Host journeys (strict pairing links, full-shell Work → Sessions → committed create/start → cancel → second-Turn append, committed approval, committed decline, Light → Dark selection and the native Android share chooser), 9 Swift tests and 7 serial iOS Simulator XCUITests covering secure pairing, Remote navigation, Sessions plus direct local search/status filtering, real loopback create/start → cancel → second-Turn append, committed approval and committed decline, Activity, the native iOS share sheet, Light → Dark → System selection, safe diagnostics, notification entry and confirmed unpair under accessibility-extra-large type. A fail-closed evidence verifier binds all 37 PNGs to the manual plus the native projects/specs/install artifacts; current Debug APK and arm64 Simulator App both pass cold-install/launch, while unsigned release outputs remain explicitly non-distributable. Close only with physical iOS/Android Gateway-to-Runtime create/reconnect/background/wake/decision/cancel/revoke evidence. |
...
| T1 Built-in tools | accepted | accepted | partial | partial | partial | active | Rust/Kotlin exact catalogue, schemas, resolvers and shared Prepared-v3 digest/access fixture are verified. Unix descriptor-confined read/list/search executors pass real path, link, case, Unicode, bound and receipt tests. Implement journaled patch, sandboxed process, non-Unix backends and live F0 composition next. |
| T2 Browser/Computer Use | accepted | accepted | partial | missing | missing | active | Rust/Kotlin execution capabilities, mandatory sandbox controls and shared canonical sandbox fixtures are verified. Implement exact Browser/Computer tool schemas and bindings, browser-native adapter contract suite, then macOS XPC Accessibility/ScreenCapture/input broker and packaged permission evidence. |
