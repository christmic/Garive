...
| A-DESKTOP-WORK | macOS local-first work product | accepted | accepted | partial | partial | partial | active | Complete shared H3 evidence, VoiceOver/real-200%-zoom/M75-M76 localization matrices and Developer-ID/notarized release gates. H1/H2/C2 sessions, exact Activity/approval detail, Workspaces and governed Artifacts are implemented; bounded strict-cursor pagination restores up to 512 Turns/256 Artifacts without silently dropping later work. Native menus now route seven closed data-free intents through the same React state machine, including bounded 80%-200% native WebView Zoom In/Out/Actual Size controls. Strict bounded preference schema v2 migrates exact v1 appearance records and supports live theme/density plus system/English/Simplified Chinese/QA-pseudolocale selection without persisting sensitive state. Stable-key localization is implemented across Setup, navigation, Home, Composer, Search, Agents, Timeline, Inspector, Activity, exact approvals, Workspace picker/recovery/revoke, Artifact preview/export, Settings, Privacy and errors; source audit leaves only product/identity marks untranslated. Focused 1280x720 Chinese and expanded-pseudolocale evidence has no horizontal overflow or console errors. Composer tests prove Enter does not submit during CJK IME composition. An effective 640x400 reflow run covers Home, Approval, Artifact overlay and scrollable Settings without horizontal overflow; it exposed and fixed unnamed collapsed-rail controls. This remains useful preflight evidence, not a claim of real macOS WebView 200% zoom. The native zoom implementation, 55/55 frontend tests, production build, full Rust tests/clippy and a debug `.app` bundle pass locally; locked-host UI automation prevented native screenshot evidence. Full M75/M76, native CJK IME, VoiceOver, real 200% zoom and packaged-candidate evidence remain open. Picker focus containment/restoration, semantic Inspector tabs and system accessibility preferences have local evidence. A hardened-runtime Universal ad-hoc DMG passes local audit, while Gatekeeper, stapling and warning-free strip remain external release gates. |
