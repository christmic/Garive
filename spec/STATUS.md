...
| H2 | Client-safe Host read model | accepted | accepted | documented | partial | partial | active | H2 wire values, exact tags and Rust/KMP/TypeScript presence semantics are verified; implement fixed-prefix Runtime discovery, Session and timeline projections. |
| H3 | Public Agent activity projection | accepted | accepted | documented | partial | partial | active | D0 snapshot v2 catalogue/digest and additive H3 wire/client mappings are verified; implement redacted committed Runtime activity snapshots/events. |
