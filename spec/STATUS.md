...
| A-DESKTOP-C2 | Secure Desktop configuration onboarding | accepted | accepted | partial | partial | partial | active | Complete expiry/idempotency, cancellation and shared crash fixtures; strict v2, redacted planning, Keychain commit, durable staged recovery/rotation cleanup, restart IPC and three-step first-run UI are implemented. |
| A-DESKTOP-WORK | macOS local-first work product | accepted | accepted | partial | partial | partial | active | Deliver H1/H2/H3/C2-backed sessions, progress, decisions and setup, then opaque Workspaces, governed effects, artifacts and packaged macOS evidence. |
