...
| A-DESKTOP-WORK | macOS local-first work product | accepted | accepted | partial | partial | partial | active | Complete shared H3 evidence, governed effects/artifacts and signed packaged macOS gates; H1/H2/C2 sessions, continuation, setup, deliverables, committed Activity, selected-file context UX and native bookmark recovery are implemented. |
| A-DESKTOP-WA | Governed Workspaces and artifacts | accepted | accepted | documented | partial | partial | active | Complete immutable artifact projection, safe preview/export, detach/revoke receipts and remaining crash/accessibility matrices; Desktop now shares one Workspace authority registry with Runtime, requires process-local read-write authorization plus an exact durable per-effect approval, performs descriptor-relative no-overwrite atomic creation, emits the complete C5 receipt chain, and proves the approval-to-artifact file flow against real SQLite. |
