...
| A-DESKTOP-WA | Governed Workspaces and artifacts | accepted | accepted | documented | partial | partial | active | Complete durable global-revoke receipts, export-crash cleanup and remaining accessibility/package matrices. Session-scoped Workspace detach now has an idempotent durable receipt, restart-safe projection and keyboard-focus-preserving Desktop flow; immutable receipt-bound Artifact projection, restart recovery, digest-verified text preview and five-minute window-bound one-shot native no-overwrite export are implemented, and the real SQLite approval-to-file-to-Artifact flow passes. |
