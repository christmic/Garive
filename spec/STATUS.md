...
| A-TUI | Resident terminal product | accepted | accepted | documented | implemented | partial | active | Keep the verified macOS and Linux arm64 Runtime/SQLite/PTTY, reducer, editor, render, persistence, H2/H3 navigation and activity flows, 30-minute release reconnect-churn gate, plus Windows/Linux x86_64 all-target check and strict-Clippy evidence green; Windows native ACL/ConPTY, Linux x86_64 native/physical-terminal/tmux/`TERM=dumb`, and final screenshot closure remain open. |
...
| A-DESKTOP-WORK | macOS local-first work product | accepted | accepted | partial | partial | partial | active | Complete shared H3 evidence, VoiceOver/real-200%-zoom/M75-M76 localization matrices and Developer-ID/notarized release gates. H1/H2/C2 sessions, exact Activity/approval detail, Workspaces and governed Artifacts are implemented; bounded strict-cursor pagination restores up to 512 Turns/256 Artifacts without silently dropping later work. Native menus route seven closed data-free intents through the same React state machine, including bounded 80%-200% native WebView Zoom In/Out/Actual Size controls. A dedicated closed locale command now rebuilds all custom and standard Garive/File/Edit/View/Window items in English, Simplified Chinese, or QA pseudolocale without changing identities or accelerators. A package-critical ACL audit found that only 7 of 29 registered product commands had been admitted; the capability/build manifest now admits the exact full set to `main` only, with an executable parity gate. Strict bounded preference schema v2 migrates exact v1 appearance records and supports live theme/density plus system/English/Simplified Chinese/QA-pseudolocale selection without persisting sensitive state. Stable-key localization is implemented across Setup, navigation, Home, Composer, Search, Agents, Timeline, Inspector, Activity, exact approvals, Workspace picker/recovery/revoke, Artifact preview/export, Settings, Privacy and errors; source audit leaves only product/identity marks untranslated. Focused 1280x720 Chinese and expanded-pseudolocale evidence has no horizontal overflow or console errors. Composer tests prove Enter does not submit during CJK IME composition. An effective 640x400 reflow run covers Home, Approval, Artifact overlay and scrollable Settings without horizontal overflow; it exposed and fixed unnamed collapsed-rail controls. This remains useful preflight evidence, not a claim of real macOS WebView 200% zoom. Native zoom/menu/ACL tests, production build, full Rust tests/clippy and a debug `.app` bundle pass locally; locked-host UI automation prevented native screenshot evidence. A fail-closed manifest/verifier derives all 62 M01-M85 rows from A-DESKTOP-VE and currently reports every row pending. The complete Chinese task-oriented manual skeleton binds each procedure and recovery branch to exactly one machine-checked pending marker; final admission requires exact candidate-bound PNG references and rejects draft text. Full M75/M76, native CJK IME, VoiceOver, real 200% zoom and packaged-candidate evidence remain open. Picker focus containment/restoration, semantic Inspector tabs and system accessibility preferences have local evidence. A hardened-runtime Universal ad-hoc DMG passes local audit, while Gatekeeper, stapling and warning-free strip remain external release gates. |
...
| A-MOBILE-R | Native remote-work mobile product | accepted | accepted | documented | implemented | partial | active | KMP controller, secure pairing, native Compose/SwiftUI product, installable builds, Gateway/private APNs/FCM wake paths, complete 25-screenshot user guide, real native loopback actions, offline/reconnect, semantic icon navigation, and dark/200%-equivalent type evidence are verified locally. Settings includes persistent theme, system notification controls, verified host and safe build/device diagnostics. Unpair now requires an explicit scope-preserving confirmation. Android Keystore and Apple Keychain tests prove grants are absent from preferences, clear makes them unrecoverable and unpair rotates device identity. Close only with physical iOS/Android Gateway-to-Runtime create/reconnect/background/wake/decision/cancel/revoke evidence. |
...
| A-UX1 | Product client experience | accepted | accepted | partial | partial | partial | active | UX-A and Desktop UX-B are complete: the work surface now runs through the shared controller with exact create/start/cancel/text-or-boolean continuation commands, atomic bounded preference/pending recovery, restart-safe multi-turn evidence, keyboard/focus/contrast/reduced-motion checks, 200% zoom and 320px reflow. Close UX-C shared Web/native UI plus remaining localization and physical-device accessibility matrices. |
