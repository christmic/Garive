...
| R1 | Local Runtime composition | accepted | accepted | documented | implemented | partial | active | Repair the reproducible `restart_abandons_unproven_execution_before_dispatch` reconstruction failure, then rerun the full recovery suite; explicit configuration, fixed-prefix starts, post-commit queue and real protocol flow remain implemented. |
...
| A-DESKTOP-WORK | macOS local-first work product | accepted | accepted | partial | partial | partial | active | Complete shared H3 evidence, durable Workspace bookmarks, governed effects/artifacts and signed packaged macOS gates; H1/H2/C2 sessions, continuation, setup, deliverables, committed Activity and selected-file context UX are implemented. |
| A-DESKTOP-WA | Governed Workspaces and artifacts | accepted | accepted | documented | partial | partial | active | Add durable macOS security-scoped bookmarks, detach/revoke receipts, governed file effects, immutable artifact projection, safe preview/export and remaining crash/accessibility matrices; process-local owner-bound grants, durable Session attachment, opaque paging and selected UTF-8 context admission are verified. |
