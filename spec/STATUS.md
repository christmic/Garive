...
| C7-C | Publication-grade context-pressure runner | accepted | accepted | missing | missing | missing | active | Implement the strict non-secret provider run document, injected OS credential reference and bounded no-retry HTTPS exchange; loopback evidence must remain non-publishable. |
