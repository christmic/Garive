...
| A-DESKTOP-WA | Governed Workspaces and artifacts | accepted | accepted | documented | partial | partial | active | Complete detach/revoke receipts, export-crash cleanup and remaining accessibility/package matrices. Immutable receipt-bound Artifact projection, restart recovery, digest-verified text preview and five-minute window-bound one-shot native no-overwrite export are implemented; the real SQLite approval-to-file-to-Artifact flow passes. |
