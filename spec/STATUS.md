...
| H1 | Live durable Host | accepted | accepted | documented | partial | partial | active | Normalize `api_version = "v1"`, add schema-validated canonical JSON continuation field 5, and prove one live-Host-to-shared-client E2E before restoring completion. |
...
| Memory M2 | accepted | accepted | missing | partial | partial | active | Migrate the initial parser from alias numeric revisions to exact M0 identities, then add Kotlin planning and the atomic namespace control journal/projection receipts. |
