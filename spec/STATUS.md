...
| C5b | Deterministic governed effect batches | accepted | accepted | documented | partial | partial | active | C5b-A exact declarations, Prepared v2, conflict graph and Rust/Kotlin planner evidence are verified; implement C5b-R bounded Runtime execution/recovery and ordered publication. |
...
| H2 | Client-safe Host read model | accepted | accepted | documented | implemented | verified | done | Keep Agent discovery, Session snapshots, bounded restart-safe timelines, typed suspension prompt digests and generated KMP mapping gates green. |
| H3 | Public Agent activity projection | accepted | accepted | documented | implemented | verified | done | Keep installed public labels and redacted committed activity projection/restart tests green. |
...
| A-DESKTOP-C2 | Secure Desktop configuration onboarding | accepted | accepted | missing | missing | missing | active | Add preset-expanded strict config v2, write-only staged setup IPC, crash recovery, credential rotation and first-run/reconfigure UI. |
...
| A-MOBILE-R | Native remote-work mobile product | accepted | accepted | documented | implemented | partial | active | KMP controller, secure pairing, native Compose/SwiftUI product, installable builds and Gateway are verified locally; close only with physical iOS/Android Gateway-to-Runtime create/reconnect/background/decision/cancel/revoke evidence. |
