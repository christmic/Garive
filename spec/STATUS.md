...
| A-DESKTOP-WORK | macOS local-first work product | accepted | accepted | partial | partial | partial | active | Complete shared H3 evidence, VoiceOver/200%-zoom/localization matrices and Developer-ID/notarized release gates. H1/H2/C2 sessions, exact Activity/approval detail, Workspaces and governed Artifacts are implemented; picker focus containment/restoration, semantic Inspector tabs, reduced motion/transparency, increased contrast and forced colors have local Browser evidence. A resource-sealed hardened-runtime Universal (`x86_64 arm64`) ad-hoc DMG passes local codesign/DMG/launch audit, while Gatekeeper, stapling and warning-free strip remain external release gates. |
...
| A-UX1 | Product client experience | accepted | accepted | partial | partial | partial | active | Close shared Web/native controller and remaining localization/accessibility evidence. Desktop now includes H3 activity and exact approvals, typed continuation, governed Artifact preview/export, distinct pending/committed Workspace context, durable detach/revoke, focus-contained native selection and explicit path-free recovery actions. |
