...
| R1 | Local Runtime composition | accepted | accepted | documented | implemented | verified | done | Keep explicit configuration, fixed-prefix start/replacement reconstruction, post-commit queue, real protocol flow, bounded shutdown and process-kill recovery gates green. |
...
| A-DESKTOP-WORK | macOS local-first work product | accepted | accepted | partial | partial | partial | active | Complete shared H3 evidence, crash/accessibility matrices and Developer-ID/notarized release gates. H1/H2/C2 sessions, exact Activity/approval detail, Workspaces and governed Artifacts are implemented; a resource-sealed hardened-runtime Universal (`x86_64 arm64`) ad-hoc DMG passes local codesign/DMG/launch audit, while Gatekeeper, stapling and warning-free strip remain external release gates. |
| A-DESKTOP-WA | Governed Workspaces and artifacts | accepted | accepted | documented | partial | partial | active | Complete export-crash cleanup and remaining accessibility/package matrices. Session detach has an idempotent durable receipt and restart-safe projection; global revoke immediately drops authority, journals a private manifest-v2 tombstone, retries failed Keychain cleanup after restart, replays `already_revoked`, and has a two-step focus-preserving Desktop flow. Immutable receipt-bound Artifact projection, digest-verified preview and one-shot no-overwrite export are implemented, and the real SQLite approval-to-file-to-Artifact flow passes. |
...
| A-UX1 | Product client experience | accepted | accepted | partial | partial | partial | active | Add governed effects/artifact presentation and close shared Web/native controller evidence; Desktop now includes H3 activity, typed continuation, a keyboard-accessible native Workspace/context flow and an explicit authorization recovery state/action with opaque file identities. |
