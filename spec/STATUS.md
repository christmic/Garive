...
| A-DESKTOP-WA | Governed Workspaces and artifacts | accepted | accepted | documented | partial | partial | active | Add reauthorization reconciliation, detach/revoke receipts, governed file effects, immutable artifact projection, safe preview/export and remaining crash/accessibility matrices; read-only macOS security-scoped bookmarks now persist only in Keychain behind a path-free atomic manifest, restore opaque grants across process rebuild, balance scoped access and expose aggregate recovery health. |
...
| A-UX1 | Product client experience | accepted | accepted | partial | partial | partial | active | Add governed effects/artifact presentation and close shared Web/native controller evidence; Desktop now includes H3 activity, typed continuation and a keyboard-accessible native Workspace/context flow with opaque file identities. |
