...
| A-WEB | Web shell | accepted | accepted | documented | implemented | verified | done | Keep the canonical React Work UI, strict H1/H2 HTTP/SSE effect port, durable navigation/timeline, all H1 mutations, bounded browser preference/pending recovery, shared task control center, 11 Web tests and production build green. Same-origin deployments must proxy `/v1`; wildcard CORS is forbidden. |
