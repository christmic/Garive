...
| A-DESKTOP-WA | Governed Workspaces and artifacts | accepted | accepted | documented | partial | partial | active | Add detach/revoke receipts, governed file effects, immutable artifact projection, safe preview/export and remaining crash/accessibility matrices; read-only macOS bookmarks persist only in Keychain behind a path-free atomic manifest, restore across process rebuild, reject root/home, preserve dormant records, and reauthorize only the same private file identity through Settings or native selection. |
