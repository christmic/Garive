...
| A-DESKTOP-WA | Governed Workspaces and artifacts | accepted | accepted | documented | partial | partial | active | Add the Desktop Workspace authority/executor, detach/revoke receipts, immutable artifact projection, safe preview/export and remaining crash/accessibility matrices; bookmarks and reauthorization are durable, and the local Runtime worker now admits Tools only through an explicit per-Execution factory with the complete C5 effect fact chain verified. |
