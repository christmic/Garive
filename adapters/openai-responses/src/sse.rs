...
impl SseDecoder {
    ...
    pub fn push(&mut self, bytes: &[u8]) -> Result<Vec<SseFrame>, ResponsesAdapterError> {
...
fn parse_frame(bytes: &[u8]) -> Result<Option<SseFrame>, ResponsesAdapterError> {
    let text = std::str::from_utf8(bytes).map_err(|_| ResponsesAdapterError::InvalidSse)?;
    let mut event = None;
        ...
        match field {
            ...
            "retry" => {
                retry = Some(
                    value
                        .parse::<u64>()
                        .map_err(|_| ResponsesAdapterError::InvalidSse)?,
                );
            }
            _ => {}
        }
